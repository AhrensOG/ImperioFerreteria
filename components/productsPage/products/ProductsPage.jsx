...
import { deleteSearchByCategoryFilter, deleteSearchByNameFilter } from "@/context/actions";
  ...
  return (
    <div className="p-4 sm:px-12 flex flex-col gap-8">
      {state?.searchedProducts?.length > 0 && state?.searchedTitle ? (
        ...
        <div className="flex flex-row justify-start items-center gap-4 py-4 sm:py-12 sm:px-12">
          <div className="relative">
            <button className="text-2xl sm:text-3xl md:text-4xl text-[#e26928] border-b border-b-[#e26928] flex flex-row items-center gap-2" onClick={() => setShowCategories(!showCategories)}>
              Categorias
              <svg
              ...
              ? <div className="flex flex-col w-full gap-2 absolute py-2">
                {
                  state?.categories?.map((c) => {
                    return <CategoryCard key={c.id} category={c} showCategories={setShowCategories}/>
                  })
                }
              </div>
